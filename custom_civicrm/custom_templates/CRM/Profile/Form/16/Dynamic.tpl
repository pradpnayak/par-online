...
{/if}
    ...
    {foreach from=$fields item=field key=fieldName}
		...
		{if $n eq 'postal_code-5'}
		  <div id="editrow-supporter" class="crm-section editrow_supporter-section form-item">
		     <div class="label">{$form.supporter.label}</div>
	    	     <div class="edit-value content">{$form.supporter.html}</div>		
		     <div class="clear"></div>
		  </div>
		{/if}
...
{literal}
  ...
  cj("#_qf_Edit_next").click( function(){  
  var denomination = '{/literal}{$denomination}{literal}';
  var denominationID = '{/literal}{$denominationID}{literal}';
  var supporter = cj( '#supporter' ).val();
  var chk = false;
  if( supporter == '' ){
    chk = confirm( "No supporter is specified. Click OK to set up a direct supporter of relationship between the donor and the "+denomination+". Click Cancel to provide a supporter." );
  if( chk == true ){
    cj( "input[name=supporter_id]" ).val(denominationID);
  } else {
    return false;
  }
