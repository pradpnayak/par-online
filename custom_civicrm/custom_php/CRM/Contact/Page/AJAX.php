...
class CRM_Contact_Page_AJAX
{
    static function getContactList( ) 
    {
        ...
        if ( !empty($pc) ) {
            $from['id'] = "LEFT JOIN civicrm_relationship as cr ON ( cc.id = cr.contact_id_b AND cr.relationship_type_id = 13)";
        }
        ...
        if ( CRM_Utils_Array::value( 'org', $_GET) ) {
            if ( !empty($pc) ) {
                $where .= " AND contact_type = \"Organization\" AND cr.is_active = 1";
            } else {
