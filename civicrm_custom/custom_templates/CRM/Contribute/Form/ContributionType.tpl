   ...
   {else}
     ...
     <table class="form-layout-compressed">
	  ...
	  <td class="html-adjust">{$form.organization_name.html|crmReplace:class:twenty}
       ...
       <tr>
	  <td class="label">{$form.parent_contribution_type.label}
	  </td>
	  <td class="html-adjust">{$form.parent_contribution_type.html|crmReplace:class:twenty}
          </td>
       </tr>
...
// remove current employer id when current employer removed.
cj("form").submit(function() {
  if ( !cj('#parent_contribution_type').val() ) cj( "#parent_id" ).val('');
});

//current employer default setting
var employerId = "{/literal}{$parentId}{literal}";
if ( employerId ) {
    var dataUrl = "{/literal}{crmURL p='civicrm/ajax/rest' h=0 q="className=CRM_Contribute_Page_AJAX&fnName=contributionType&json=1&parentID=" }{literal}" + employerId ;
    cj.ajax({ 
        url     : dataUrl,   
        async   : false,
        success : function(html){
            //fixme for showing address in div
            htmlText = html.split( '|' , 2);
            cj('input#parent_contribution_type').val(htmlText[0]);
            cj('input#parent_id').val(htmlText[1]);
        }
    }); 
}

cj("input#parent_contribution_type").click( function( ) {
