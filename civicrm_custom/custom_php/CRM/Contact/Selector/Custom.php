    ...
    function &getRows($action, $offset, $rowCount, $sort, $output = null) {
        ...
        while ( $dao->fetch( ) ) {
            ...
            if ( ! $empty ) {
                ...
                if ( !empty( $contactID ) ) {
                    $contact_type_a = CRM_Contact_BAO_Contact::getContactType( $contactID );
                    $contactSubType = CRM_Contact_BAO_Contact::getContactSubType( $contactID );
                    if ( $contact_type_a ==  'Individual' ) {
                        if ( $links[2]['name'] == 'Edit' ) {
                            $links[2]['url'] = 'civicrm/profile/edit';
                            $links[2]['qs']  = 'reset=1&gid='.CONTACT_PROFILE_ID.'&id='.$contactID;
                            $links[10] = array('name' => 'Donation',
                                               'url'  => 'civicrm/contact/view',
                                               'qs'   => 'reset=1&cid='.$contactID.'&selectedChild=donation' );
                        }
                    } else if ( $contact_type_a ==  'Organization' ) {
                        if ( $links[2]['name'] == 'Edit' ) {
                            $links[2]['url'] = 'civicrm/profile/edit';
                            if ( $contactSubType == 'Pastoral_Charge') {
                                $links[2]['qs']  = 'reset=1&gid='.PC_PROFILE_ID.'&id='.$contactID;
                            } else if ( $contactSubType == 'Denomination') {
                                $links[2]['qs']  = 'reset=1&gid='.DENOMINATION_PROFILE_ID.'&id='.$contactID;
                            } else if ( $contactSubType == 'Congregation') {
                                $links[2]['qs']  = 'reset=1&gid='.CO_PROFILE_ID.'&id='.$contactID;
                            } else {
                                $links[2]['qs']  = 'reset=1&gid='.ORGANIZATION_PROFILE_ID.'&id='.$contactID;
                            }
                ...
                // Quick Charge Search - Start
                if ( !empty($dao->non_uc_name) && empty($dao->pc_name) ) {
                    $row['pc_name'] = $dao->non_uc_name;
                }
                if ( !empty($dao->non_uc_name) && empty($dao->deno_name) ) {
                    $row['deno_name'] = $dao->non_uc_name;
                } // Quick Charge Search - End
                $rows[] = $row;
            }
        }
        return $rows;
