...
$read  = fopen( '/home/mayur/Desktop/donorimport.csv', 'r' );
$newRecordsToInsert  = fopen('/home/mayur/Desktop/donorimport.sql', 'w' );
        ...
        { 
            ...
            if ( !empty( $donor_cong_amount ) || $donor_cong_amount != 0 ) {
                ...
                if ( ! empty( $contribution_cong_rec )) {
                    $contribution_cong = " INSERT INTO civicrm_contribution ( contact_id, contribution_type_id, payment_instrument_id, receive_date, total_amount, fee_amount, net_amount, currency, contribution_recur_id, contribution_status_id , is_test) values ((SELECT MAX(id) FROM civicrm_contact where external_identifier ='{$ext_id}' ), (SELECT MAX(id) FROM civicrm_contribution_type where name = 'Congregation' AND contact_id = (SELECT MAX(id) FROM civicrm_contact where external_identifier ='{$donor_owner_id}' )  ), 6, '2011-10-20 00:00:00', {$donor_cong_amount}, {$fee_amount}, {$net_amount}, 'CAD', ( SELECT MAX(id) FROM civicrm_contribution_recur ), 5, 0 );\n ";
...
$read  = fopen( '/home/mayur/Desktop/donorimport.csv', 'r' );
$newRecordsToInsert  = fopen('/home/mayur/Desktop/donorrelimport.sql', 'w' );
...
while ( $rows = fgetcsv( $read ) ) { 
    ...
    if ( !empty( $ext_id ) && !empty( $donor_owner_id ) ) {
        $insert_donor = "INSERT INTO civicrm_relationship ( contact_id_a, contact_id_b, relationship_type_id, is_active) values ((SELECT MAX(id) FROM civicrm_contact where external_identifier ='{$ext_id}'),(SELECT MAX(id) FROM civicrm_contact where external_identifier ='{$donor_owner_id}'),13,1);\n";
    }
